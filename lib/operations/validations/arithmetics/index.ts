...
export function isGte(path: string, n: number, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
...
export function isGt(path: string, n: number, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
    const _error = new BrickError(error || `%s is not greater then ${n}`, 'isGt');
    return generateBrick( _isGt, [n], path, _error, ifPass );
}


export function isLte(path: string, n: number, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
    const _error = new BrickError(error || `%s is not less or equals then ${n}`, 'isLte');
    return generateBrick( _isLte, [n], path, _error, ifPass );
}


export function isLt(path: string, n: number, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
    const _error = new BrickError(error || `%s is not less then ${n}`, 'isLt');
    return generateBrick( _isLt, [n], path, _error, ifPass );
}


export function isBetween(path: string, a: number, b: number, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
...
export function isPositive(path: string, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
...
export function isNegative(path: string, ifPass?: IfPassFn, error?: string): AsyncBrickFn {
