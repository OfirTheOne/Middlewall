...
export function isGte(path: string, n: number, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
    const _error = new BrickError(`%s is not greater or equals then ${n}`, 'isGte');
    return generateBrick( _isGte, [n], path, _error, ifPass );
}

export function isGt(path: string, n: number, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
    const _error = new BrickError(`%s is not greater then ${n}`, 'isGt');
    return generateBrick( _isGt, [n], path, _error, ifPass );
}


export function isLte(path: string, n: number, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
    const _error = new BrickError(`%s is not less or equals then ${n}`, 'isLte');
    return generateBrick( _isLte, [n], path, _error, ifPass );
}


export function isLt(path: string, n: number, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
    const _error = new BrickError(`%s is not less then ${n}`, 'isLt');
    return generateBrick( _isLt, [n], path, _error, ifPass );
}


export function isBetween(path: string, a: number, b: number, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
    const _error = new BrickError(`%s is not between ${a} and ${b}`, 'isBetween');
    return generateBrick( _isBetween, [a,b], path, _error, ifPass );
}


export function isPositive(path: string, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
...
export function isNegative(path: string, ifPass?: IfPassFn, error?: ErrorConstructor | string): AsyncBrickFn {
